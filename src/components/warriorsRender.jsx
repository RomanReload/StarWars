...
const WarriorsBlock = (props) => {
  const { warriorsArr, listsButton = 0, setListstButton } = props;
  let pageNumber = listsButton
  let partsOfCards = [];
    ...
    ({ name, eye_color, gender, birth_year }) => {
      ...
      return (
        <>
          <div
            key={uniqueId(name)}
            className="col-6 col-sm-6 col-md-4 card warrior-card mb-1"
            style={{ width: "10rem" }}
          >
            <img
              src={imgUrl}
              className="card-img-top img-fluid mt-1 rounded"
              alt="..."
            />
